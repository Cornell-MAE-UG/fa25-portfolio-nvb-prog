...
Problem Definition: In Homework 4, we were tasked with designing a frame mechanism to lift the max posible load to the highest possible height.

Constraints and Objectives:
- 2D space of 150cm long and 50cm tall
- 3 pin supports of which two need to be mounted on the ground and a linear actuator
- Maximize the height achevied with the rigid bar mounted design

Design Degrees of Freedom:
- Rigid bar of a fixed length
- Linear actuator from the Tolomatic catalog

Solving the Problem: 
I implemented a computational approach to analyze the static behavior of the mechanism. The analysis defined feasible ranges for two key design variables:
- Distance between the pivot point and the linear actuator attachment location along the rigid bar
- Angle that the rigid bar forms with the ground during operation

These variables were discretized using a step size of 0.01, and the code iterated through all combinations to evaluate the torque balance about the pivot. At each iteration, the algorithm calculated the maximum load that could be supported under static equilibrium and recorded the corresponding actuator position and bar angle. This approach enabled identification of the configuration that produced the maximum supported load.

Several simplifying assumptions were made to streamline the analysis. The linear actuator was assumed to act at a 90-degree angle relative to the rigid bar, as this orientation maximizes the torque generated about the pivot for a given actuator force. Additionally, the length of the rigid bar was fixed at 100cm to reduce the number of unknown parameters. The IMA actuator from the Tolomatic catalog was selected because it was the only actuator for which complete performance data were available and because it geometrically fit within the design constraints in its unextended configuration.

Final Design and Results

The analysis showed that the maximum load the mechanism could support was 35.045 kN. This maximum occurs when the linear actuator is positioned at the furthest allowable distance from the pivot (100cm), with the rigid bar forming an angle of 16.7° relative to the ground. This result is consistent with physical intuition: when the actuator force acts perpendicular to the bar, increasing the actuator’s moment arm increases the generated torque, thereby maximizing the load that can be lifted against gravity.
